...
def join_predict_split(df_ref_window, df_test_window):
    ...
    df_union_index_reset = df_union.reset_index()
    print('df_union index', df_union_index_reset.index)

    # predict their label values
    predicted_labels = KMeans(n_clusters=2, random_state=0).fit_predict(df_union_index_reset)

    # split values by predicted label and window
    return split_back_to_windows(df_union_index_reset, predicted_labels, len(df_ref_window), len(df_test_window))
