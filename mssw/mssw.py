...
def obtain_cluster_distances_and_sizes(weighted_sub_window, fitted_kmeans, num_clusters):
    """
    ...
    :param num_clusters: number of clusters used to fit the kmeans object
    :return: (array of shape (1, num_clusters) of sums of centroid distances,
    array of shape (1, num_clusters) of cluster sizes)
    """
    centroids = fitted_kmeans.cluster_centers_
    predicted_cluster_labels = fitted_kmeans.predict(weighted_sub_window)

    centroid_distance_sums = np.zeros(num_clusters).reshape((1, num_clusters))
...
def calculate_clustering_statistics(weighted_sub_window, fitted_kmeans, num_clusters):
    ...
    centroid_distance_sums, num_points_in_clusters = obtain_cluster_distances_and_sizes(
        weighted_sub_window, fitted_kmeans, num_clusters
    )
